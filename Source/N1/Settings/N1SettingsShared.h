...
#include "N1SettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EN1AllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
class UN1LocalPlayer;

/**
 * UN1SettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UN1SettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

	public:
	DECLARE_EVENT_OneParam(UN1SettingsShared, FOnSettingChangedEvent, UN1SettingsShared* Settings);
...
public:
	...
	static UN1SettingsShared* CreateTemporarySettings(const UN1LocalPlayer* LocalPlayer);

	/** Synchronously loads a settings object, this is not valid to call before login */
	static UN1SettingsShared* LoadOrCreateSettings(const UN1LocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UN1SettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UN1LocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EN1AllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EN1AllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EN1AllowBackgroundAudioSetting AllowAudioInBackground = EN1AllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EN1GamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EN1GamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EN1GamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EN1GamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();

private:
	UPROPERTY()
	EN1GamepadSensitivity GamepadLookSensitivityPreset = EN1GamepadSensitivity::Normal;
	UPROPERTY()
	EN1GamepadSensitivity GamepadTargetingSensitivityPreset = EN1GamepadSensitivity::Normal;
