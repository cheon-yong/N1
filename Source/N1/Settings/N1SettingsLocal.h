...
#include "N1SettingsLocal.generated.h"

enum class ECommonInputType : uint8;
enum class EN1DisplayablePerformanceStat : uint8;
enum class EN1StatDisplayMode : uint8;
...
USTRUCT()
struct FN1ScalabilitySnapshot
{
	GENERATED_BODY()

	FN1ScalabilitySnapshot();
 ...
 */
UCLASS()
class UN1SettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EN1StatDisplayMode GetPerfStatDisplayState(EN1DisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EN1DisplayablePerformanceStat Stat, EN1StatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UN1SettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EN1DisplayablePerformanceStat, EN1StatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FN1ScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FN1ScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UN1SettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UN1SettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	...
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UN1LocalPlayer* LocalPlayer);
	...
	void ResetKeybindingToDefault(const FName MappingName, UN1LocalPlayer* LocalPlayer);
	...
	void ResetKeybindingsToDefault(UN1LocalPlayer* LocalPlayer);
