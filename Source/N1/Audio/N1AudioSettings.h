...
#include "N1AudioSettings.generated.h"
...
USTRUCT()
struct N1_API FN1SubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "N1AudioSettings"))
class N1_API UN1AudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FN1SubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FN1SubmixEffectChainMap> LDRAudioSubmixEffectChain;
