...
#include "N1AudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FN1AudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class N1_API UN1AudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the N1 Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the N1 Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the N1 Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the N1 Audio Settings and linked to the UI and game settings in N1SettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the N1 Audio Settings and linked to the UI and game settings in N1SettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the N1 Audio Settings and linked to the UI and game settings in N1SettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the N1 Audio Settings and linked to the UI and game settings in N1SettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the N1 Audio Settings and linked to the UI and game settings in N1SettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FN1AudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FN1AudioSubmixEffectsChain> LDRSubmixEffectChain;
