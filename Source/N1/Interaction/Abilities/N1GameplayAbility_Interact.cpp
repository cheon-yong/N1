...
#include "N1GameplayAbility_Interact.h"
...
#include "UI/IndicatorSystem/N1IndicatorManagerComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(N1GameplayAbility_Interact)
...
UN1GameplayAbility_Interact::UN1GameplayAbility_Interact(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ActivationPolicy = EN1AbilityActivationPolicy::OnSpawn;
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
	NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;
}

void UN1GameplayAbility_Interact::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UN1GameplayAbility_Interact::UpdateInteractions(const TArray<FInteractionOption>& InteractiveOptions)
{
	if (AN1PlayerController* PC = GetN1PlayerControllerFromActorInfo())
	{
		if (UN1IndicatorManagerComponent* IndicatorManager = UN1IndicatorManagerComponent::GetComponent(PC))
		{
...
void UN1GameplayAbility_Interact::TriggerInteraction()
{
