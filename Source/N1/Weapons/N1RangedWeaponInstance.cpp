...
#include UE_INLINE_GENERATED_CPP_BY_NAME(N1RangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_N1_Weapon_SteadyAimingCamera, "N1.Weapon.SteadyAimingCamera");

UN1RangedWeaponInstance::UN1RangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UN1RangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UN1RangedWeaponInstance::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UN1RangedWeaponInstance::UpdateDebugVisualization()
{
...
void UN1RangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UN1RangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UN1RangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UN1RangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UN1RangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UN1RangedWeaponInstance::AddSpread()
{
...
float UN1RangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UN1RangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UN1RangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UN1RangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UN1CameraComponent* CameraComponent = UN1CameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_N1_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
