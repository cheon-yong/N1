...
#include "N1ReticleWidgetBase.h"

#include "Inventory/N1InventoryItemInstance.h"
#include "Weapons/N1RangedWeaponInstance.h"
#include "Weapons/N1WeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(N1ReticleWidgetBase)

UN1ReticleWidgetBase::UN1ReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UN1ReticleWidgetBase::InitializeFromWeapon(UN1WeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UN1InventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UN1ReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UN1RangedWeaponInstance* RangedWeapon = Cast<const UN1RangedWeaponInstance>(WeaponInstance))
	{
...
bool UN1ReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UN1RangedWeaponInstance* RangedWeapon = Cast<const UN1RangedWeaponInstance>(WeaponInstance))
	{
...
float UN1ReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
