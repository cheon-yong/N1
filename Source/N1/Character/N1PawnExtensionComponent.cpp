...
#include UE_INLINE_GENERATED_CPP_BY_NAME(N1PawnExtensionComponent)
...
void UN1PawnExtensionComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(UN1PawnExtensionComponent, PawnData);
}
...
void UN1PawnExtensionComponent::OnRegister()
{
	...
	ensureAlwaysMsgf((Pawn != nullptr), TEXT("N1PawnExtensionComponent on [%s] can only be added to Pawn actors."), *GetNameSafe(GetOwner()));

	TArray<UActorComponent*> PawnExtensionComponents;
	Pawn->GetComponents(UN1PawnExtensionComponent::StaticClass(), PawnExtensionComponents);
	ensureAlwaysMsgf((PawnExtensionComponents.Num() == 1), TEXT("Only one N1PawnExtensionComponent should exist on [%s]."), *GetNameSafe(GetOwner()));
...
void UN1PawnExtensionComponent::SetPawnData(const UN1PawnData* InPawnData)
{
	...
	if (PawnData)
	{
		UE_LOG(LogN1, Error, TEXT("Trying to set PawnData [%s] on pawn [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(Pawn), *GetNameSafe(PawnData));
		return;
	}
...
void UN1PawnExtensionComponent::OnRep_PawnData()
{
	CheckDefaultInitialization();
}
...
void UN1PawnExtensionComponent::InitializeAbilitySystem(UN1AbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	...
	UE_LOG(LogN1, Verbose, TEXT("Setting up ASC [%s] on pawn [%s] owner [%s], existing [%s] "), *GetNameSafe(InASC), *GetNameSafe(Pawn), *GetNameSafe(InOwnerActor), *GetNameSafe(ExistingAvatar));

	if ((ExistingAvatar != nullptr) && (ExistingAvatar != Pawn))
	{
		UE_LOG(LogN1, Log, TEXT("Existing avatar (authority=%d)"), ExistingAvatar->HasAuthority() ? 1 : 0);

		// There is already a pawn acting as the ASC's avatar, so we need to kick it out
		// This can happen on clients if they're lagged: their new pawn is spawned + possessed before the dead one is removed
		ensure(!ExistingAvatar->HasAuthority());

		if (UN1PawnExtensionComponent* OtherExtensionComponent = FindPawnExtensionComponent(ExistingAvatar))
		{
...
void UN1PawnExtensionComponent::HandleControllerChanged()
{
...
void UN1PawnExtensionComponent::HandlePlayerStateReplicated()
{
	CheckDefaultInitialization();
}
...
void UN1PawnExtensionComponent::HandleChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState)
{
	if (DesiredState == N1GameplayTags::InitState_DataInitialized)
	{
...
void UN1PawnExtensionComponent::OnAbilitySystemInitialized_RegisterAndCall(FSimpleMulticastDelegate::FDelegate Delegate)
{
...
void UN1PawnExtensionComponent::OnAbilitySystemUninitialized_Register(FSimpleMulticastDelegate::FDelegate Delegate)
{
