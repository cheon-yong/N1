...
#include UE_INLINE_GENERATED_CPP_BY_NAME(N1ContextEffectsLibrary)


void UN1ContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UN1ContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UN1ContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UN1ContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FN1ContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UN1ActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FN1ContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UN1ActiveContextEffects* NewActiveContextEffects = NewObject<UN1ActiveContextEffects>(this);
	...
	this->N1ContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UN1ContextEffectsLibrary::N1ContextEffectLibraryLoadingComplete(
	TArray<UN1ActiveContextEffects*> N1ActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(N1ActiveContextEffects);
}
