...
#include "Feedback/ContextEffects/N1ContextEffectsSubsystem.h"
#include "Kismet/GameplayStatics.h"
#include "NiagaraFunctionLibrary.h"
#include "NiagaraSystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(N1ContextEffectsSubsystem)
...
void UN1ContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UN1ContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UN1ContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UN1ContextEffectsLibrary* EffectLibrary : EffectsLibraries->N1ContextEffectsLibraries)
			{
...
bool UN1ContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UN1ContextEffectsSettings* ProjectSettings = GetDefault<UN1ContextEffectsSettings>())
	{
...
void UN1ContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UN1ContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UN1ContextEffectsSet* EffectsLibrariesSet = NewObject<UN1ContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UN1ContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UN1ContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->N1ContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UN1ContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
