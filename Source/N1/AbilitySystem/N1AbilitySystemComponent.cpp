...
void UN1AbilitySystemComponent::InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor)
{
	...
	{
		if (UN1AnimInstance* AnimInst = Cast<UN1AnimInstance>(ActorInfo->GetAnimInstance()))
		{
...
void UN1AbilitySystemComponent::ProcessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const UN1GameplayAbility* AbilityCDO = CastChecked<UN1GameplayAbility>(AbilitySpec->Ability);
				if (AbilityCDO->ActivationPolicy == EN1AbilityActivationPolicy::WhileInputActive)
				{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputPressedSpecHandles)
	{
		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				{
					const UN1GameplayAbility* AbilityCDO = CastChecked<UN1GameplayAbility>(AbilitySpec->Ability);
					if (AbilityCDO->ActivationPolicy == EN1AbilityActivationPolicy::OnInputTriggered)
					{
						AbilitiesToActivate.AddUnique(AbilitySpec->Handle);
					}
				}
