...
void UN1DamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	FN1GameplayEffectContext* TypedContext = FN1GameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UN1TeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UN1TeamSubsystem>();
	...
	if (const IN1AbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
