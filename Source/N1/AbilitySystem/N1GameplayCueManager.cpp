...
#include "N1GameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(N1GameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EN1EditorLoadMode
{
...
namespace N1GameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("N1.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via N1GameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UN1GameplayCueManager::DumpGameplayCues));

	static EN1EditorLoadMode LoadMode = EN1EditorLoadMode::LoadUpfront;
}
...
void UN1GameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UN1GameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogN1, Warning, TEXT("UN1GameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UN1GameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (N1GameplayCueManagerCvars::LoadMode)
	{
	case EN1EditorLoadMode::LoadUpfront:
		return true;
	case EN1EditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EN1EditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UN1GameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UN1GameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UN1GameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UN1GameplayCueManager* GCM = Cast<UN1GameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogN1, Error, TEXT("DumpGameplayCues failed. No UN1GameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogN1, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogN1, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogN1, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogN1, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogN1, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogN1, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogN1, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogN1, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogN1, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogN1, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogN1, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogN1, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UN1GameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UN1GameplayCueManager* StrongThis = Get())
					{
...
void UN1GameplayCueManager::HandlePostGarbageCollect()
{
...
void UN1GameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogN1, Warning, TEXT("UN1GameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UN1GameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (N1GameplayCueManagerCvars::LoadMode)
	{
	case EN1EditorLoadMode::LoadUpfront:
...
void UN1GameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UN1GameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UN1GameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UN1GameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UN1GameplayCueManager::ShouldDelayLoadGameplayCues() const
{
